...
parse_IPC_MS_project_names <- function(rawData_folder, matrix, parseRunOrder=FALSE) {
  ...
  if (parseRunOrder == TRUE) {
      ...
      meta_files <- data.frame(sampleFileName=tmp_sampleFileName, sampleType = tmp_sampleType, acquisitionTime = acqTime, stringsAsFactors = FALSE)
      meta_files <- meta_files[origRunOrder , ]
      meta_files$RunOrder <- seq(1, length(origRunOrder))
      ...
      for (idx in 1:length(time_delta)) {
          if (time_delta[idx] > 72) {
              cBatch <- cBatch + 1
          }

         batchVec[idx + 1] <- cBatch
      }
      meta_files$Batch <- batchVec

   all_files <- all_files[origRunOrder]
