...
def create_annotation_methods():
    ...
    task = CreateAnnotationMethod(annotation_method_name="Bi-Quant-U",
                                  description='Bruker IVDR BI-QUANT Urine on NMR',
                                  username=USERNAME,
                                  db_env=DB_ENV)
    task.run()

    task = CreateAnnotationMethod(annotation_method_name="Bi-Quant-P",
                                  description='Bruker IVDR BI-QUANT Plasma on NMR',
                                  username=USERNAME,
                                  db_env=DB_ENV)
    task.run()
...
def import_devset_lpos_peakpanther_annotations():
    ...
    task = ImportPeakPantherAnnotations(project_name=PROJECT_NAME,
                                        ...
                                        assay_name="LPOS",
                                        db_env=DB_ENV,
                                        #run_batch_correction='LTR',
                                       )

    return task.run()

def import_devset_project_lpos_peakpanther_annotations(project_name):
    from phenomedb.imports import ImportPeakPantherAnnotations
    ...
    task = ImportPeakPantherAnnotations(project_name=PROJECT_NAME,
                                        ...
                                        sample_metadata_csv_path=config['DATA'][
                                                                     'test_data'] + 'DEVSET P LPOS PeakPantheR_sampleMetadata.csv',
